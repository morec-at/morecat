...
- RMU(Rust): `cd apps/rmu` で `cargo build` / `cargo test`。line / region coverage は `cargo llvm-cov --locked --all-features --fail-under-lines 100 --fail-under-regions 100`（HTML は `cargo llvm-cov --html --open`）。Rust stable では LLVM branch coverage が未安定のため region coverage を用いる。
