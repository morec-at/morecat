...
- `DATABASE_USER`: Postgres read-model user
- `DATABASE_PASSWORD`: Postgres read-model user password
...
Postgres に `articles` read-model table を用意したうえで、同じ dev shell 内の2つのターミナルを使う。先にターミナル1で Firestore emulator を起動する。
...
DATABASE_URL=jdbc:postgresql://127.0.0.1:5432/morecat \
DATABASE_USER=morecat \
