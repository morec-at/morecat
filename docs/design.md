...
| RMU（読取モデル更新） | **Scala.js → Node**。`domain.js`（cross-compile した fold/codec）を API と共有。イベントを fold して Cloud SQL に upsert（冪等・順序無関係）。Node 採用で高速コールドスタート。**upsert 完了後に viewer へ revalidate 発火** | **Cloud Run**（API とは別サービス） |
