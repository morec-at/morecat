...
- `ArticleDrafted` — 下書き作成（初期 slug/title/body。tags は slice2 以降に追加）
...
### ドメイン表現
- 値オブジェクトは **Iron** で精緻化（Slug / Title 等）+ smart constructor。ただし集約 ID は表現非依存の不透明型（`ArticleId`）とし、UUIDv7 の採番・形式検証は infrastructure に置く
...
- **UUIDv7**: DB 往復なしにアプリ側で ID 採番でき、ヘクサゴナルの自律生成と相性が良い（採番自体は時計・乱数を要するため infrastructure の責務。domain は表現非依存の不透明 ID として扱う）。時刻順で index 効率も良好。
