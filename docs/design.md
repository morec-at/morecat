...
```
[API (Cloud Run)] --append event--> [Firestore (Native) = イベントストア]
                                            |
                                     doc 作成イベント
                                            v
                                     [Eventarc トリガ]
...
### RMU（読み取りモデル更新）
- **Scala.js → Node on Cloud Run**（高速コールドスタート）。`domain.js`（cross-compile した fold / codec）を API と共有。Firestore 読取・Postgres 書込は Node ライブラリ（`@google-cloud/firestore`, `pg`）の Scala.js facade
...
| RMU（読取モデル更新） | **Scala.js → Node**。`domain.js`（cross-compile した fold/codec）を API と共有。イベントを fold して Cloud SQL に upsert（冪等・順序無関係）。Node 採用で高速コールドスタート | **Cloud Run**（API とは別サービス） |
...
```
build.sbt                   単一 sbt ビルド（root が全 Scala モジュールを集約）
project/
modules/
  domain/                   crossProject(.jvm/.js): Article イベント ADT・値オブジェクト(Iron)・projection fold(純粋)・JSON codec(zio-json)
apps/
  api/                      JVM deployable: tapir HTTP サーバ（Cloud Run）
    application/            ユースケース（コマンド/クエリ）※ JVM
    infrastructure/         Firestore(JVM SDK)・Postgres(JDBC)・GCS・tapir
    bootstrap/              ZLayer 配線・エントリポイント
  rmu/                      Scala.js deployable: Eventarc 起動の RMU（Node on Cloud Run）
    infrastructure/         Firestore / pg の Scala.js facade
    bootstrap/              エントリポイント
  ui/
    web/
...
- 単一 sbt ビルド。共有は `modules/domain` の crossProject（`.jvm` / `.js`）のみ。`apps/api` は `domain.jvm`、`apps/rmu` は `domain.js` に依存
- `application` / `infrastructure` は API 固有（JVM）として `apps/api` 配下、RMU 固有の infra は `apps/rmu` 配下。sbt サブプロジェクトの base dir で参照
...
- **RMU = Scala.js → Node（JVM ではなく）**: ephemeral な購読者として都度起動するため、JVM のコールドスタート（数秒）を避けたい。Scala.js なら Node 上で高速起動しつつ、`domain`（イベント定義・fold・codec）を cross-compile で API と**共有**できる。共有は純粋 domain のみに限定し、IO アダプタは各プラットフォーム（JVM SDK / Node facade）で個別実装 ＝「ドメインは1つの真実、IO は各ランタイム」。これにより Scala は単一 sbt ビルド（crossProject）に確定し、api/rmu は別 deployable のまま。
